...
log "codex setup"
"$BIN" agent install codex --patch > "$SMOKE_DIR/codex-install.json"
"$BIN" agent install codex --patch > "$SMOKE_DIR/codex-install-again.json"
assert_file "$SMOKE_DIR/AGENTS.md"
assert_file "$SMOKE_DIR/.codex/config.toml"
assert_contains '"patched": true' "$SMOKE_DIR/codex-install.json"
assert_contains '"patched": false' "$SMOKE_DIR/codex-install-again.json"
assert_count "<!-- meshlet:codex:start -->" 1 "$SMOKE_DIR/AGENTS.md"
assert_count "<!-- meshlet:codex:end -->" 1 "$SMOKE_DIR/AGENTS.md"
assert_count "[mcp_servers.meshlet]" 1 "$SMOKE_DIR/.codex/config.toml"
"$BIN" agent show codex > "$SMOKE_DIR/codex-show.json"
assert_contains '"agents_md_section_present": true' "$SMOKE_DIR/codex-show.json"
